...
from utils import fine_line, get_employee, money_and_warning_summary, sync_parent_links_from_crm
...
async def my_children_start(message: Message) -> None:
    async with async_session() as session:
        ...
        if not links:
            if await sync_parent_links_from_crm(session, employee):
                result = await session.execute(
                    select(ParentLink).where(ParentLink.employee_id == employee.id)
                )
                links = result.scalars().all()
...
async def my_children_schedule(message: Message) -> None:
    async with async_session() as session:
        ...
        links = result.scalars().all()
        if not links and await sync_parent_links_from_crm(session, employee):
            result = await session.execute(
                select(ParentLink).where(ParentLink.employee_id == employee.id)
            )
            links = result.scalars().all()
